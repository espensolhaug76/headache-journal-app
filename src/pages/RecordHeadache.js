    ...
    return (
      <div style={{
        ...
        <div style={{ maxWidth: '500px', width: '100%' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
            <button
              onClick={()/* eslint-disable no-unused-vars */
      ...
      if (isEditMode && editDocId) {
        ...
        if (existingData) {
          setFormData(prev => ({
            ...
            isMigrineAttack: existingData.isMigrineAttack || false, // NEW: Load migraine status
  ...
  const startHeadacheSession = async () => {
    ...
    try {
      const sessionData = {
        ...
        isMigrineAttack: formData.isMigrineAttack, // NEW: Store migraine status
        ended: false,
  ...
  const endHeadacheSession = async () => {
    ...
    try {
      ...
      const headacheData = {
        ...
        isMigrineAttack: ongoingSession.isMigrineAttack, // NEW: Include migraine status
    ...
    try {
      if (isEditMode && editDocId) {
        ...
        const success = await updateRecord({
          painLevel: parseInt(formData.painLevel),
          location: formData.location,
          isMigrineAttack: formData.isMigrineAttack, // NEW: Include migraine status
          date: formData.date,
          ...(isPremiumMode && {
            prodromeSymptoms: formData.prodromeSymptoms,
            currentSymptoms: formData.currentSymptoms,
        ...
        console.log('Migraine attack:', formData.isMigrineAttack);
